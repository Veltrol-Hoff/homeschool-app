        ...
        <div className="space-y-12">
          ...
          <section>
            <h2 className="text-xl font-bold text-stone-900 mb-4 flex items-center gap-2">
              <LucideIcons.Globe2 className="text-emerald-500" />
          ...
          {(() => {
            ...
            const futureTrips = trips.filter((t: any) => t.start_date > now)
            const pastTrips = trips.filter((t: any) => t.start_date <= now)
            ...
            return (
              <>
                {futureTrips.length > 0 && (
                  <section>
                    <h2 className="text-xl font-bold text-stone-900 mb-4 flex items-center gap-2">
                      <LucideIcons.CalendarClock className="text-blue-500" />
                      Upcoming Trips
                    </h2>
                    <div className="space-y-4">
                      {futureTrips.map((trip: any) => <TripCard key={trip.id} trip={trip} />)}
                    </div>
                  </section>
                )}
                ...
                {pastTrips.length > 0 && (
                  <section>
                    <h2 className="text-xl font-bold text-stone-900 mb-4 flex items-center gap-2">
                      <LucideIcons.History className="text-stone-500" />
                      Past Trips
                    </h2>
                    <div className="space-y-4">
                      {pastTrips.map((trip: any) => <TripCard key={trip.id} trip={trip} />)}
                    </div>
                  </section>
                )}
              </>
